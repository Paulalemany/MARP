...
protected:
   ...
   struct TreeNode {
      ...
      Link iz, dr;
      int altura;
      int tam_i;    //Guardamos el numero de nodos del hijo izquierdo mas uno
...
protected:
    ...
    T const& kesimo(int K, Link raiz) const 
    {
       //El tam_i de la raiz tiene su posicion en el inorden por lo tanto
       //Si k < tam_i el numero estara a la izquierda de la raiz
       //Si k > tam_i el numero estara a la derecha de la raiz

        if (raiz == nullptr) //El elemento no existe
            throw std::domain_error("El elemento no existe");

        if (K == raiz->tam_i)    //El elemento que buscamos es la raiz
            return raiz->elem;
        if (K < raiz->tam_i) {

            return kesimo(K, raiz->iz);
        }
        else if (K > raiz->tam_i) {
            //Le restamos el tam_i de la raiz actual para que pueda comparar sin tenerlos en cuenta
   ...
   //Debemos mantener el tam_i actualizado
   bool inserta(T const& e, Link & a) {
      ...
      if (a == nullptr) { // se inserta el nuevo elemento e
         ...
         crece = true;
         a->tam_i = 1;
      } else if (menor(e, a->elem)) {   //Lo inserta en el hijo izquierdo
         crece = inserta(e, a->iz);
         if (crece) {
             a->tam_i++;   //Aumenta en 1 el tamaño solo si crece
             reequilibraDer(a);
         }

      } else if (menor(a->elem, e)) {   //Inserta en el hijo derecho
