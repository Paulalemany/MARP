...
template <typename U, typename T = int, typename Comparator = std::less<T>>
...
private:
   ...
   std::unordered_map<U, size_t> posiciones;   // un 0 indica que el elemento no está
...
public:
   ...
   IndexPQ(IndexPQ<U,T,Comparator> const&) = default;

   IndexPQ<U,T,Comparator>& operator=(IndexPQ<U,T,Comparator> const&) = default;
   ...
   void pop() {
      if (size() == 0) throw std::domain_error("No se puede eliminar el primero de una cola vacía.");
          ...
          U elem = array[1].elem;   //Elemento que queremos eliminar
          posiciones.erase(elem);   //El propio mapa no hace nada si no está
